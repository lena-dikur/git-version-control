...
Чтобы в полной мере использовать возможности git нужен текстовый формат документов и MS Word для этих целей не подходит. Мы будем использовать Markdown - это простой язык текстовой разметки. Он предназначен для создани красиво оформленных текстов.
...
Запушить "протолкнуть" созданные локально изменения на удаленный репозиторий можно с помощью команды:
...
"Стянуть" изменения с удаленного репозитория, а затем слить с текущей веткой на свой локальный компьютер, можно с помощью команды:

    git pull

## Работа над open source проектом

Чтобы поучаствовать в проекте к которому нет доступа, необходимо сделать копию репозитория, для этого используется кнопка Fork. Появитяся копия репозитория на нашем аккаунте. Теперь, скопировав ссылку можно забрать репозиторий на свой компьютер. Набрав команду git push, система подскажет дальнейшие действия:


    git push --set -upstream origin remoted

На сайте GitHub нужно нажать появившуюся кнопку Pull Request.
